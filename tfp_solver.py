...
class tfp_solver(object):
    ...
    def initialize(self, h_sim, N, M, u_ig, x_ig, y_ig):
        ...
        self.x = ca.MX.sym('x', self.tfp.x_dim)
        self.u = ca.MX.sym('u', self.tfp.u_dim)

        # Time grid for simulation
        self.tgrid_sim = np.linspace(self.tfp.t_0, self.tfp.t_f, round((self.tfp.t_f - self.tfp.t_0)/self.h_sim)+1)


    def plot_trajectory(self, step_u=False):
        plt.figure(1)
        legend = []
        for i in range(self.tfp.x_dim):
            plt.plot(self.tgrid_sim, self.x_opt[i,:].T, '-')
            legend.append('x_'+ str(i+1) + ': ' + self.tfp.x_dict[str(i+1)])
        for i in range(self.tfp.u_dim):
            if not step_u:
                plt.plot(self.tgrid_sim[0:-1], self.u_opt[i,:].T, '-')
            else:
                plt.step(self.tgrid_sim[0:-1], self.u_opt[i,:].T, '-', where='post')
            legend.append('u_'+ str(i+1) + ': ' + self.tfp.u_dict[str(i+1)])
        plt.xlabel('t')
        plt.legend(legend)
        plt.grid()
        plt.show()
...
class tfp_template_solver(tfp_solver):
    ...
    def __init__(self, tfp):
        ...
        super(tfp_template_solver, self).__init__(tfp)


    def initialize_ca(self):
        self.h_N = (self.tfp.t_f - self.tfp.t_0)/self.N
        self.tgrid_N = np.linspace(self.tfp.t_0, self.tfp.t_f, self.N+1)

        self.x_dot = []
        for element in self.tfp.x_dot:
    ...
    def get_trajectory(self):
        self.x_opt = np.reshape(self.tfp.x_0, (self.tfp.x_dim,1))
        for i in range(len(self.tgrid_sim)-1):
            x_opt_i = self.simulator(x0=self.x_opt[:,-1], p=self.u_opt[:,i])
            self.x_opt = np.append(self.x_opt, np.reshape(np.array(x_opt_i['xf'].full()), (self.tfp.x_dim,1)), axis=1)



class tfp_ssm_solver(tfp_template_solver):


    def __init__(self, tfp):

        super(tfp_ssm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, N, u_ig=None):
        ...
        # Formulate the NLP
        Xk = ca.MX(self.tfp.x_0)
        for k in range(self.N):
            # New NLP variable for the control
            Uk = ca.MX.sym('U_' + str(k), self.tfp.u_dim)
            self.w.append(Uk)
            self.lbw.append(self.tfp.u_min)
            self.ubw.append(self.tfp.u_max)
            if u_ig is None:
                self.w0.append(np.zeros(self.tfp.u_dim))
            else:
                self.w0.append(self.u_ig[:,k])

            # Integrate till the end of the interval
            Xk = self.I(x0=Xk, p=Uk)['xf']

            # Add inequality constraint
            self.g.append(Xk)
            if k < self.N-1:
                self.lbg.append(self.tfp.x_min)
                self.ubg.append(self.tfp.x_max)
            else:
                self.lbg.append(self.tfp.x_f)
                self.ubg.append(self.tfp.x_f)
        ...
        u_opt_N = np.reshape(sol['x'], (self.tfp.u_dim,self.N))
...
class tfp_msm_solver(tfp_template_solver):


    def __init__(self, tfp):

        super(tfp_msm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, N, u_ig=None, x_ig=None):
        ...
        for k in range(self.N):
            ...
            # Integrate till the end of the interval
            Xk_end = self.I(x0=Xk, p=Uk)['xf']

            # Add inequality constraint
            if k == self.N-1:
                Xk = ca.MX(self.tfp.x_f)
            else:
                Xk = ca.MX.sym('X_' + str(k+1), self.tfp.x_dim)
                self.w.append(Xk)
                self.lbw.append(self.tfp.x_min)
                self.ubw.append(self.tfp.x_max)
                if x_ig is None:
                    self.w0.append(np.zeros(self.tfp.x_dim))
                else:
                    self.w0.append(self.x_ig[:,k])
            self.g.append(Xk_end - Xk)
            self.lbg.append(np.zeros(self.tfp.x_dim))
            self.ubg.append(np.zeros(self.tfp.x_dim))
        ...
        for i in range(self.tfp.u_dim):
            u_opt_N = np.append(u_opt_N, w_opt[i::self.tfp.u_dim+self.tfp.x_dim])
        u_opt_N = np.reshape(u_opt_N, (self.tfp.u_dim,self.N))
...
class tfp_cm_solver(tfp_template_solver):


    def __init__(self, tfp):

        super(tfp_cm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, N, M, u_polynomial, u_continuous=False, u_ig=None, x_ig=None):
        ...
        f = ca.Function('f', [self.x, self.u], [self.x_dot], ['x', 'u'], ['xdot'])
        ...
        for k in range(self.N):
            ...
            if u_polynomial:
                Uc = []
                for j in range(self.M):
                    Ukj = ca.MX.sym('U_' + str(k) + '_' + str(j), self.tfp.u_dim)
                    Uc.append(Ukj)
                    self.w.append(Ukj)
                    self.lbw.append(self.tfp.u_min)
                    self.ubw.append(self.tfp.u_max)
            ...
            Xc = []
            for j in range(self.M):
                Xkj = ca.MX.sym('X_' + str(k) + '_' + str(j), self.tfp.x_dim)
                Xc.append(Xkj)
                self.w.append(Xkj)
                self.lbw.append(self.tfp.x_min)
                self.ubw.append(self.tfp.x_max)
            ...
            if u_polynomial and u_continuous:
                ...
                if k > 0:
                    ...
                    self.lbg.append(np.zeros(self.tfp.u_dim))
                    self.ubg.append(np.zeros(self.tfp.u_dim))
        ...
        if u_polynomial:
            u_opt_N = np.zeros((self.tfp.u_dim,0))
            for i in range(self.N):
                u_opt_N = np.append(u_opt_N, w_opt[(self.M+1)*self.tfp.x_dim*i + self.M*self.tfp.u_dim*i:(self.M+1)*self.tfp.x_dim*i + self.M*self.tfp.u_dim*(i+1)])
            u_opt_N = np.reshape(u_opt_N, (self.tfp.u_dim,self.N*self.M))

            t = sp.symbols('t')
            self.u_opt = np.zeros((self.tfp.u_dim,0))
            for i in range(self.N):
                u_t_i = 0
                for j in range(self.M):
            ...
            self.u_opt = np.reshape(self.u_opt, (self.tfp.u_dim,len(self.tgrid_sim)-1))
            ...
            u_opt_N = np.zeros((self.tfp.u_dim,0))
            for i in range(self.N):
                u_opt_N = np.append(u_opt_N, np.reshape(w_opt[(self.M+1)*self.tfp.x_dim*i + self.tfp.u_dim*i:(self.M+1)*self.tfp.x_dim*i + self.tfp.u_dim*(i+1)], (self.tfp.u_dim,1)), axis=1)
            u_opt_N = np.reshape(u_opt_N, (self.tfp.u_dim,self.N))
...
class ocp_i_solver(tfp_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_i_solver, self).__init__(tfp)


    def initialize_ca(self):
        # setup bvp
        y = sp.Matrix(sp.symbols('y[:'+str(self.tfp.x_dim)+']'))

        H = self.tfp.L + y.T*self.tfp.x_dot
        y_dot = sp.simplify(-H.jacobian(self.tfp.x))
        Hu = sp.simplify(H.jacobian(self.tfp.u))

        u_noc = sp.solve(Hu, self.tfp.u)
        u_noc = sp.Matrix([val for (key, val) in u_noc.items()])
        z_dot = sp.Matrix([*self.tfp.x_dot, *y_dot]).subs([(self.tfp.u[i], u_noc[i]) for i in range(self.tfp.u_dim)])


        self.y = ca.MX.sym('y', self.tfp.x_dim)

        self.u_opt = []
        for element in u_noc:
            element_str_replaced = str_replace_all(str(element), ca_replacements)
            self.u_opt.append(eval(element_str_replaced))
        self.u_opt = ca.vertcat(*self.u_opt)

        if self.tfp.u_min > -np.inf*np.ones(self.tfp.u_dim):
            self.u_opt = ca.fmax(self.u_opt, self.tfp.u_min)
        if self.tfp.u_max < np.inf*np.ones(self.tfp.u_dim):
            self.u_opt = ca.fmin(self.u_opt, self.tfp.u_max)

        self.z = ca.vertcat(self.x, self.y)

        self.z_dot = []
        for element in z_dot:
            element_str_replaced = str_replace_all(str(element), ca_replacements)
    ...
    def get_trajectory(self):
        ufcn = self.u_opt_fcn.map(len(self.tgrid_sim))
        self.u_opt = ufcn(self.z_opt)[0:-1]
        self.x_opt = np.zeros((0,len(self.tgrid_sim)))
        for i in range(self.tfp.x_dim):
            self.x_opt = np.append(self.x_opt, self.z_opt[i,:])
        self.x_opt = np.reshape(self.x_opt, (self.tfp.x_dim,len(self.tgrid_sim)))
...
class ocp_issm_solver(ocp_i_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_issm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, y_ig=None):
        ...
        # Create an integrator (CVodes)
        I = ca.integrator('I', 'cvodes', self.ode, {'t0': self.tfp.t_0, 'tf': self.tfp.t_f, 'abstol': self.int_tol, 'reltol': self.int_tol})

        y_0 = ca.MX.sym('y_0', self.tfp.x_dim)
        z_0 = ca.vertcat(self.tfp.x_0, y_0)

        Z = I(x0=z_0)['xf']
        x_f = Z[0:self.tfp.x_dim] - self.tfp.x_f
        ...
        self.z_opt = self.simulator(x0=np.concatenate((self.tfp.x_0, y_0_opt)))['xf']
...
class ocp_imsm_solver(ocp_i_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_imsm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, N, x_ig=None, y_ig=None):
        ...
        # Create an integrator (CVodes)
        I = ca.integrator('I', 'cvodes', self.ode, {'t0': self.tfp.t_0, 'tf': self.tfp.t_f/self.N, 'abstol': self.int_tol, 'reltol': self.int_tol})

        # Variables in the root finding problem
        V = ca.MX.sym('V', (self.N+1)*2*self.tfp.x_dim)

        Z = []
        for i in range(self.N+1):
            Z.append(V[i*2*self.tfp.x_dim:(i+1)*2*self.tfp.x_dim])

        # Formulate the root finding problem
        G = []
        G.append(Z[0][0:self.tfp.x_dim] - self.tfp.x_0)
        for k in range(self.N):
            Z_end = I(x0=Z[k])['xf']
            G.append(Z_end - Z[k+1])
        G.append(Z[self.N][0:self.tfp.x_dim] - self.tfp.x_f)
        ...
        self.z_opt = self.simulator(x0=z_opt[0:2*self.tfp.x_dim])['xf']
...
class ocp_icm_solver(ocp_i_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_icm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, N, M, x_ig=None, y_ig=None):
        ...
        # Variables in the root finding problem
        V = ca.MX.sym('V', (self.N*(self.M+1)+1)*2*self.tfp.x_dim)
        ...
        G.append(Z[self.N*(self.M+1)][0:self.tfp.x_dim] - self.tfp.x_f)
...
class ocp_d_solver(tfp_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_d_solver, self).__init__(tfp)
...
class ocp_dssm_solver(ocp_d_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_dssm_solver, self).__init__(tfp)
...
class ocp_dmsm_solver(ocp_d_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_dmsm_solver, self).__init__(tfp)
...
class ocp_dcm_solver(ocp_d_solver):
    ...
    def __init__(self, tfp):
        ...
        super(ocp_dcm_solver, self).__init__(tfp)
    ...
    def solve(self, h_sim, N, M, u_polynomial, u_continuous=False, u_ig=None, x_ig=None):
        ...
        # Continuous time dynamics
        f = ca.Function('f', [self.x, self.u], [self.x_dot, self.L], ['x', 'u'], ['xdot', 'L'])
...
if __name__ is '__main__':
    ...
    problem_names = {'1': 'double_int', '2': 'simple_pend', '3': 'pend_cart_pl', '4': 'pend_cart', '5': 'dual_pend_cart_pl',
                     '6': 'dual_pend_cart', '7': 'vtol', '8': 'ua_manipulator_pl', '9': 'acrobot_pl'}
...
#    dssm_solver = ocp_dssm_solver(tfp1)
#    dssm_solver.solve(h_sim, N)

#    dmsm_solver = ocp_dmsm_solver(tfp1)
#    dmsm_solver.solve(h_sim, N)

#    dcm_solver = ocp_dcm_solver(tfp1)
#    dcm_solver.solve(h_sim, N, M, False, False)

#    issm_solver = ocp_issm_solver(tfp1)
#    issm_solver.solve(h_sim)

#    imsm_solver = ocp_imsm_solver(tfp1)
#    imsm_solver.solve(h_sim, N)

#    icm_solver = ocp_icm_solver(tfp1)
#    icm_solver.solve(h_sim, N, M)

#    ssm_solver = tfp_ssm_solver(tfp1)
#    ssm_solver.solve(h_sim, N)

#    msm_solver = tfp_msm_solver(tfp1)
#    msm_solver.solve(h_sim, N)

#    cm_solver = tfp_cm_solver(tfp1)
#    cm_solver.solve(h_sim, N, M, False, False)